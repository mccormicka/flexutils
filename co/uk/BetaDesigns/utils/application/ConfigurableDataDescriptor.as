package co.uk.BetaDesigns.utils.application
{
	...
	/**
	 * Custom class so that allows you to pass in the name of the 
	 * child tags rather than having to call them child. 
	 *
	 */
	public class ConfigurableDataDescriptor implements ITreeDataDescriptor
	{
		private var _childName:String;
		public function ConfigurableDataDescriptor( childName:String )
		{
			_childName = childName;
		}
	    // The getChildren method requires the node to be an Object
	    // with a children field so we change it here to allow for a custom
	    // child name.
	    ...
	    {	    	
	        ...
	        {
	            if (node is Object) {
	                if(node[ _childName ] is ArrayCollection){
	                    return node[ _childName ];
	                }else{
	                    return new ArrayCollection(node[ _childName ]);
	                }
	            }
	        }
	        catch (e:Error) {
	    ...
	    // Object with a _childName field.
	    ...
	    public function isBranch( node:Object, model:Object=null ):Boolean 
	    {
	        try {
	            if (node is Object) 
	            {
	                if (node[ _childName ] != null)  
	                {
	                	if( node[ _childName ] is ArrayCollection )
	                	{
	                		if( ( node[ _childName ] as ArrayCollection ).length > 0 )
	                		{
	                			return true;
	                		}
	                		return false
	    ...
	    public function addChildAt( parent:Object, child:Object, index:int, model:Object=null ):Boolean 
	    {
	        ...
	        if (!parent) {
	            ...
	            if (parent[ _childName ] != null) {
	                if(parent[ _childName ] is ArrayCollection) {
	                    parent[ _childName ].addItemAt(child, index);
	                    if (model){
	                    ...
	                    parent[ _childName ].splice(index, 0, child);
	                    if (model)
	                        model.dispatchEvent(event);
	                    return true;
	                }
	    ...
	    public function removeChildAt( parent:Object, child:Object, index:int, model:Object=null ):Boolean
	    {
	        ...
	        {
	            if (parent[ _childName ] != undefined)
	            {
	                parent[ _childName ].splice(index, 1);
	                if (model) 
	                    model.dispatchEvent(event);
	                return true;
	            }
